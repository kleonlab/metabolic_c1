# Kleon CFM - Arc Challenge 2025

> This is the repository for the Kleon CFM - Arc Challenge 2025 submission which is based on the STATE repository from the Arc Institute.

## Setup

Download the repository and install the dependencies:
```bash
git clone git@github.com:kleonlab/st.git
cd st
uv sync
```

Download pretrained STATE models:

```bash
cd st/assets
git lfs install
git clone https://huggingface.co/arcinstitute/ST-Tahoe
```
...
```bash
chmod +x scripts/train-sm.sh
./scripts/train-sm.sh
```

This will train a STATE model for 40000 steps and save the checkpoint to the `experiments/dry_run/` directory.

Slurm training run:

```bash
./scripts/train-state-submit.sh
```


# STATE Original Readme
