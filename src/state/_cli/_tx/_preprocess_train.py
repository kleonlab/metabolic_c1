...
def run_tx_preprocess_train(adata_path: str, output_path: str, num_hvgs: int):
    ...
    # Use seurat_v3 method for large datasets - it's much faster
    if adata.n_obs > 100000:
        logger.info(f"Large dataset detected ({adata.n_obs:,} cells). Using seurat_v3 method for faster HVG computation")
        sc.pp.highly_variable_genes(adata, n_top_genes=num_hvgs, flavor='seurat_v3')
    else:
        sc.pp.highly_variable_genes(adata, n_top_genes=num_hvgs)
    logger.info(f"HVG computation complete. Found {adata.var.highly_variable.sum()} highly variable genes")

    logger.info("Storing highly variable genes in .obsm['X_hvg']")
    adata.obsm["X_hvg"] = adata[:, adata.var.highly_variable].X.toarray()
    logger.info(f"Stored X_hvg with shape {adata.obsm['X_hvg'].shape}")
