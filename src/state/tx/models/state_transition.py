...
class StateTransitionPerturbationModel(PerturbationModel):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        cfg = kwargs.get("metrics_loss_cfg", {})  # optional nested block in your YAML
        self._pds_tau: float = cfg.get("pds_tau", 0.5)                   # softmax temp for InfoNCE
        self._w_pds: float = cfg.get("w_pds", 0.0)                       # weight for PDS-InfoNCE
        self._w_cos: float = cfg.get("w_cos", 0.0)                       # weight for cosine(logFC) loss
        self._w_l1: float = cfg.get("w_l1", 0.0)                         # weight for L1/Huber(logFC)
        self._use_huber: bool = cfg.get("use_huber", False)              # switch L1→Huber
        self._huber_delta: float = cfg.get("huber_delta", 1.0)           # Huber delta
        self._use_log1p_for_pb: bool = cfg.get("use_log1p_for_pb", True) # log1p in pseudobulk
        self._eps: float = 1e-8                                          # numerical stability

        # Cache gene name to index mapping
        self._gene_name_to_idx: Optional[Dict[str, int]] = None
        if self.gene_names is not None:
            self._gene_name_to_idx = {name: i for i, name in enumerate(self.gene_names)}
    ...
    def get_target_gene_indices(self, batch: dict, padded: bool = True) -> torch.Tensor:
        """Fast O(B) lookup using cached dict."""
        device = self._module_device()
        pert_names = batch["pert_name"]
        
        # Fast batch conversion
        if isinstance(pert_names, torch.Tensor):
            pert_names = pert_names.cpu().numpy().tolist()
            pert_names = [str(p) for p in pert_names]
        elif pert_names and isinstance(pert_names[0], (list, tuple)):
            pert_names = [p[0] for p in pert_names]
        
        # Infer B
        S = self.cell_sentence_len
        if "pert_cell_emb" in batch:
            B = max(1, batch["pert_cell_emb"].shape[0] // S) if padded else 1
        else:
            B = max(1, len(pert_names) // S) if padded else 1
        
        # Get first name per sentence
        sent_names = [pert_names[i * S] for i in range(B)] if padded else [pert_names[0]]
        
        # Fast dict lookup
        if self._gene_name_to_idx:
            idxs = [self._gene_name_to_idx.get(name, -1) for name in sent_names]
        else:
            idxs = [-1] * B
        
        return torch.tensor(idxs, dtype=torch.long, device=device)

    # ===== Helpers for metric-aligned losses =====
    ...
    def _build_exclude_mask(
        ...
        """
        Returns a boolean mask of shape [B, D] where True = keep, False = exclude.
        Safe against -1 and out-of-range indices. If nothing to exclude, returns None.
        """
        # If a full mask is provided, just sanitize and return it
        if "exclude_gene_mask" in batch:
            mask = batch["exclude_gene_mask"].to(device)
            mask = mask.bool()
            # If mask is [B, S, D], reduce over S (keep a gene only if all S agree to keep)
            if mask.dim() == 3:
                mask = mask.all(dim=1)
            # If mask is [S, D], promote to [B, D] by repeating or by taking the first row
            if mask.dim() == 2 and mask.size(0) != batch_size:
                if mask.size(0) == 1:
        ...
        # Otherwise, try to use exclude_gene_idx [B]
        if "exclude_gene_idx" not in batch:
            return None

        idx = batch["exclude_gene_idx"].to(device).long()
        # If idx came in as [B, 1] or [B, S], squeeze or reduce to [B]
        if idx.dim() > 1:
            # prefer first position per sentence
            idx = idx[..., 0]

        # Build default mask = all True
        mask = torch.ones((batch_size, gene_dim), dtype=torch.bool, device=device)

        # Valid indices: 0 <= idx < gene_dim
        valid = (idx >= 0) & (idx < gene_dim)
        if valid.any():
            rows = torch.arange(batch_size, device=device)[valid]
            cols = idx[valid]
            mask[rows, cols] = False  # exclude only valid positions

        # If none are valid, we just keep the all-True mask (exclude nothing)
        return mask


    def _pseudobulk_logfc(
        self,
        mat_pred: torch.Tensor,     # [B, S, D]
        mat_true: torch.Tensor,     # [B, S, D]
        mat_ctrl: torch.Tensor,     # [B, S, D]
        use_log1p: bool = True,
        eps: float = 1e-8,
    ) -> tuple[torch.Tensor, torch.Tensor]:
        """
        Returns (pred_delta, true_delta) as 'logFC-like' pseudobulk deltas of shape [B, D].
        We do: pb = mean over S; then log1p(pb) if requested; then delta = pb - ctrl_pb.
        """
        pred_pb = mat_pred.mean(dim=1)  # [B, D]
        true_pb = mat_true.mean(dim=1)  # [B, D]
        ctrl_pb = mat_ctrl.mean(dim=1)  # [B, D]

        if use_log1p:
            # Clamp to non-negative before log1p to avoid NaNs even if model emits tiny negatives pre-ReLU
            pred_pb = torch.log1p(torch.clamp(pred_pb, min=0.0))
            true_pb = torch.log1p(torch.clamp(true_pb, min=0.0))
            ctrl_pb = torch.log1p(torch.clamp(ctrl_pb, min=0.0))

        pred_delta = pred_pb - ctrl_pb
        true_delta = true_pb - ctrl_pb
        return pred_delta, true_delta


    def _apply_gene_mask_inplace(
        self,
        x: torch.Tensor,           # [B, D]
        mask_keep: Optional[torch.Tensor],  # [B, D] bool or None
    ):
        """Zeroes out entries where mask_keep == False (i.e., excluded genes)."""
        if mask_keep is not None:
            x *= mask_keep  # zero the excluded genes in-place
    ...
    def _cosine_loss(
        ...
        cos = (x_norm * y_norm).sum(dim=-1)  # [B]
        return (1.0 - cos).mean()


    def _huber_loss(self, x: torch.Tensor, y: torch.Tensor, delta: float = 1.0) -> torch.Tensor:
        """
        Smooth L1 (Huber) loss averaged over batch and features.
        """
        diff = x - y
        abs_diff = diff.abs()
        quad = 0.5 * (diff ** 2)
        lin = delta * (abs_diff - 0.5 * delta)
        out = torch.where(abs_diff <= delta, quad, lin)
        return out.mean()
    ...
    def training_step(self, batch: Dict[str, torch.Tensor], batch_idx: int, padded=True) -> torch.Tensor:
        ...
        if confidence_pred is not None:
            ...
            confidence_weight = 0.01  # You can make this configurable
        ...
        # === EARLY EXIT if all metric weights are zero ===
        has_metric_losses = (self._w_pds > 0.0 or self._w_cos > 0.0 or self._w_l1 > 0.0)
        if not has_metric_losses:
            return total_loss
        
        # Build exclude idx only when needed
        if "exclude_gene_idx" not in batch:
            batch["exclude_gene_idx"] = self.get_target_gene_indices(batch, padded=padded)
        
        # Choose tensors for metric losses
        use_nb = isinstance(self.gene_decoder, NBDecoder)
        
        if use_nb:
            mu, _theta = self.gene_decoder(pred)
            pred_for_metrics = mu
            true_for_metrics = batch.get("pert_cell_counts", target).reshape_as(mu)
            ctrl_for_metrics = batch.get("ctrl_cell_counts", batch["ctrl_cell_emb"]).reshape_as(mu)
        else:
            pred_for_metrics = pred
            true_for_metrics = target
            ctrl_for_metrics = batch["ctrl_cell_emb"].reshape_as(pred)
        
        B, S, D = pred_for_metrics.shape
        device = pred_for_metrics.device
        
        # Build exclude mask
        mask_keep = self._build_exclude_mask(B, D, device, batch)
        
        # Pseudobulk deltas
        pred_delta, true_delta = self._pseudobulk_logfc(
            pred_for_metrics, true_for_metrics, ctrl_for_metrics,
            self._use_log1p_for_pb, self._eps
        )
        
        self._apply_gene_mask_inplace(pred_delta, mask_keep)
        self._apply_gene_mask_inplace(true_delta, mask_keep)
        
        # Guard against NaNs
        pred_delta = torch.nan_to_num(pred_delta, posinf=1e6, neginf=-1e6)
        true_delta = torch.nan_to_num(true_delta, posinf=1e6, neginf=-1e6)
        
        # Compute losses only if weight > 0
        if self._w_pds > 0.0:
            # Faster pairwise L1 distance
            pred_exp = pred_delta.unsqueeze(1)  # [B, 1, D]
            true_exp = true_delta.unsqueeze(0)  # [1, B, D]
            dists_l1 = (pred_exp - true_exp).abs().sum(dim=-1)  # [B, B]
            
            logits = -self._pds_tau * dists_l1
            labels = torch.arange(B, device=device)
            pds_loss = F.cross_entropy(logits, labels)
            total_loss = total_loss + self._w_pds * pds_loss
            self.log("train/pds_infonce", pds_loss)
        
        if self._w_cos > 0.0:
            cos_loss = self._cosine_loss(pred_delta, true_delta, self._eps)
            total_loss = total_loss + self._w_cos * cos_loss
            self.log("train/cos_logfc", cos_loss)
        
        if self._w_l1 > 0.0:
            if self._use_huber:
                l1_loss = self._huber_loss(pred_delta, true_delta, self._huber_delta)
            else:
                l1_loss = F.l1_loss(pred_delta, true_delta)
            total_loss = total_loss + self._w_l1 * l1_loss
            self.log("train/l1_logfc", l1_loss)
