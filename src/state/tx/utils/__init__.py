...
def get_loggers(
    ...
    if use_wandb:
        try:
            ...
            # Check for existing wandb run ID to resume
            wandb_path_file = join(output_dir, name, "wandb_path.txt")
            resume_id = None
            if os.path.exists(wandb_path_file):
                with open(wandb_path_file, "r") as f:
                    wandb_path = f.read().strip()
                    # Extract run ID from path (format: entity/project/run_id)
                    resume_id = wandb_path.split("/")[-1] if wandb_path else None
                    if resume_id:
                        print(f"Found existing wandb run ID: {resume_id}. Resuming...")
            ...
            wandb_logger = WandbLogger(
                ...
                id=resume_id,  # Pass the run ID if resuming
                resume="allow" if resume_id else None,  # Allow resuming if ID exists
            )
            if cfg is not None:
